...
def parse_args():
    ...
    parser.add_argument(
        "--benchmark-suite",
        action="store_true",
        help="Run built-in benchmark suite: web, ads, hashmap, graph, matrix, cache.",
    )
    parser.add_argument(
        "--workload-file",
        action="append",
...
def main():
    ...
    print(f"  Working set stability: {classes['working_set_stability']:.2f}")
    print(f"  Temporal locality: {classes['temporal_locality']}")
    print(f"  Spatial locality: {classes['spatial_locality']}")
    ...
    try:
        ...
        plot_heatmap(sim.dram.pages + sim.cxl.pages, save_path="heatmap.png")
        plot_access_distribution(sim.chameleon, save_path="access_distribution.png")
        plot_tier_composition(sim, save_path="tier_composition.png")
        plot_latency_comparison(sim, save_path="latency_comparison.png")
        write_html_report(sim, output_path="tpp_report.html")
        print("Plots saved to hit_rate.png, latency.png, memory_usage.png, migrations.png, reuse_distance.png, working_set.png, heat_distribution.png, heatmap.png, access_distribution.png, tier_composition.png, latency_comparison.png")
